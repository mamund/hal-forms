...
All HAL-FORMS documents MUST be valid https://tools.ietf.org/html/rfc7159[JSON] documents. A valid HAL-FORMS document has two top-level objects: +_links+ and +_templates+. The following is a summary of the structure of the HAL-FORMS media type.
...
+_links+::
  Contains the list of valid +link+ objects in the HAL-FORMS document. This is a RECOMMENDED element. If this element is missing or if the contents is unrecognized, it SHOULD be treated as if it contains a single +link+ element with the key of +self+ and the value of the URL used to request this HAL-FORMS document.
+link+::
  The +link+ element consists of a key and a set of properties. 
  +
  * +key+ : The unique identifier of the +link+ object. This is a REQUIRED element. If this is missing, this +link+ object SHOULD be ignored. For this release, the only valid valud for the +key+ element is +self+. 
  * +href+ : The URL associated with the +key+. This is a REQUIRED element. If this is missing, set to empty or unparsable, this +link+ object SHOULD be ignored.

=== The +_templates+ Element
The +_templates+ element describes the state transition details including the HTTP method and arguments for the transition. This is a REQUIRED element. If the HAL-FORMS document does not contain this element or the contents are unrecognized or unparsable, the HAL-FORMS document SHOULD be ignored.

The +_templates+ element contains a list of valid +template+ objects. Each +template+ object contains the following possible properties: 
...
+key+ :: 
  The unqiue identifier for this +template+ object. This is a REQUIRED element. If this is missing, set to empty or unparsable, this +template+ object SHOULD be ignored. The value of this +key+ SHOULD match the +rel+ value in the associated HAL document that was used to retrive this HAL-FORMS document.
+method+ ::
...
+title+:: A human-readable string that can be used to identify this template. This is a valid JSON string. This is an OPTIONAL element. If it does not exist or is unparsable, consumers MAY use the +key+ value of the template as the value of +title+.
...
[NOTE]
====
The HAL media type already defines support for parameterized HTTP GET queries using URITemplates. However, the HAL-FORMS document can be treated as an alternate implementation for GET queries that include additional constraints including prompts, regular expression validators, etc.
====
...
While it is completely up to authors and consumers to determine how they wish to use the HAL-FORMS media type, the following is a suggested process flow for runtime use of HAL-FORMS documents on the Web.
 ...
 . When a HAL-FORMS document is returned by the server, clients use this information to render an input UI for humans to deal with at the appropriate time.
...
After accepting and parsing the HAL response, a HAL-FORMS compliant client app MAY use the +rel+ URLs to make requests for HAL-FORMS documents. In the case of this example, the +http://api.example.org/rels/create+ is used to see if there is a HAL-FORMS document available.

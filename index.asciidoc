...
== Encoding Requests
Once the client application has used the HAL Forms document to render a UI for accepting inputs (and the user has supplied the inputs), that same document SHOULD be used to encode a request to send to the server to excecute the state transition described by the HAL Forms document. There are two ways in which HAL Forms documents can be used to construct parameterized requests. The first is by <<encoding-request-urls, encoding request URLs>> for HTTP GET, DELETE, and HEAD requests. The second is by <<encoding-request-bodies, encoding request bodies>> for HTTP PUT, POST, and PATCH requests. 
...
[NOTE]
====
The HAL media type already defines support for paramterized HTTP GET queries using URITemplates. However, the HAL Form document can be treated as an alternate implementation for GET queries that include additional constraints including prompts, regular expression validators, etc.
====

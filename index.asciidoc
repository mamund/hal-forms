...
This is an OPTIONAL property. If this property is not understood by the recipient, left blank, or contains an invalid URL string, it SHOULD be ignored. In that case, the target URL for requests to be submitted is the +href+ attribute of the +self+ link contained in the <<links-element,+_links+ collection>>. If neither a +target+ is specified not a +self+ link can be found, the template SHOULD be ignored.

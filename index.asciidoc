...
+properties+ ::
  An array of one or more anonymous +property+ objects (see <<property-object, The +property+ Object>>) that each describe a parameter for the associated state transition. This is an OPTIONAL element. If the array is missing or empty, the +properties+ collection MUST be treated as an empty set of parameters -- meaning that the transition is meant to be executed without passing any parameters.
+property+ ::
  A JSON object that describes a state transition element. See <<property-object, The +property+ Object>> for details.
...
[[property-object]]
==== The +property+ Object
A +property+ object describes the details of the state transition element (+name+, +required+, +readOnly+, etc.). Most attributes of a +property+ object are optional. Some attributes of a property are dependent on others and/or will appear in groups. For example, the +min+ attribute is expected to appear along with the +max+ attribute.  Below is a list of all the possible attributes of a +property+ object.

...
+link+::
  ...
  +
  * +key+ : The unique identifier of the +link+ object. This is a REQUIRED element. For this release, the only valid valid for the +key+ element is +"self"+. If this element is missing, is set to an unrecognized value, or cannot be parsed, this +link+ object SHOULD be ignored.

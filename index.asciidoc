...
+property+ ::
  A JSON object that describes a state transition element. See <<property-element, The +property+ Element>> for details.
...
A +property+ element describes the details of the state transition element (+name+, +required+, +readOnly+, etc.). It appears as an array of +properties+ as a child of the +_templates+ element (See <<templates-element, The +_templates+ Element>>). Most attributes of a +property+ object are optional. Some attributes of a property are dependent on others and/or will appear in groups. For example, the +min+ attribute is expected to appear along with the +max+ attribute.  

There is a set of +property+ attributes that are considered _core_ attributes. There are is also group of +property+ attributes that are considered _additional_ attributes. Any library supporting the *HAL-FORMS* specification SHOULD support all of the _core_ attributes and MAY support some or all of the _additional_ attributes.

==== Core +property+ Attributes
Below is a list of all the _core_ +property+ attributes.

+name+:: The +property+ name. This is a valid JSON string. This is a REQUIRED element. If this attribute is missing or set to empty, the client SHOULD ignore this +property+ object completely.
...
+value+:: The +property+ value. This is a valid JSON string. This string MAY contain a URI Template (see +templated+ for details). This is an OPTIONAL element. If it does not exist, clients SHOULD act as if the +value+ property is set to an empty string.

==== Additional +property+ Attributes
Below is a list of all the _additional_ +property+ attributes.

+cols+:: The +cols+ attribute specifies the expected maximum number of characters per line to display when rendering the input box. This attribute applies to the associated +property+ element when that +property+ element's +type+ attribute is set to +"textarea"+. The +cols+ attribute is a non-negative integer greater than zero. If the +cols+ attribute is missing or contains an invalid value, it can be assumed to be set to the value of +40+. If the +type+ attribute of the associated +property+ element is not set to +"textarea"+, this attribute SHOULD be ignored. This is an OPTIONAL attribute and it MAY be ignored. The +cols+ attribute SHOULD appear along with the +rows+ attribute.
+max+:: The +max+ attribute specifies the maximum numeric value for the +value+ setting of a +property+  element. This attribute SHOULD appear along with the +min+ attribute. This is an OPTIONAL property and it MAY be ignored.
+maxLength+:: The +maxLength+ attribute specifies the maximum number of characters allowed in the +value+ property. This attribute SHOULD appear along with the +minLength+ attribute. This is an OPTIONAL property and it MAY be ignored.
+min+:: The +min+ attribute specifies the minimum numeric value for an +value+ setting of a +property+ element. This attribute SHOULD appear along with the +max+ attribute. This is an OPTIONAL property and it MAY be ignored.
+minLength+:: The +minlength+ attribute specifies the minimum number of characters required in a +value+ property. this attribute SHOULD appear along with the +maxLength+ attribute. This is an OPTIONAL property and it MAY be ignored.

...
A +property+ element describes the details of the state transition element (+name+, +required+, +readOnly+, etc.). It appears as an array of +properties+ as a child of the +_templates+ element (See <<templates-object, The +_templates+ Element>>). Most attributes of a +property+ object are optional. Some attributes of a property are dependent on others and/or will appear in groups. For example, the +min+ attribute is expected to appear along with the +max+ attribute.  Below is a list of all the possible attributes of a +property+ object.

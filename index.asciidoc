...
[[property-object]]
==== The +property+ Object
A +property+ object describes the details of the state transition element (+name+, +readOnl+, etc.). Most attributes of a +property+ object are optional. Some attributes of a property are dependent on others and/or will appear in groups. For example, +min+ attribute is expected to appear along with the +max+ attribute.  Below is a list of all the possible attributes of a +property+ object.
